<?php
		...
		if((isset($_SESSION["name"]))&&(isset($_REQUEST["value"])))
		{
			...
			echo 
			"<div class ='anurag'>
			<table border='2' style='width=200px;position:absolute;top:130px;left:8px;'>
			<tr>
			<td>Wrong receipt</td>
			...
			{
				if(($arr["approval"]==0)&&($arr["Query"]==0))
				{
					echo
					"
					<tr>
					<td>
						<input type = 'radio' name ='event$recptno' id =$recptno onclick='Query_events(this.id);' value = 1>Query</input>
					</td>
						<td>$recptno</td>
						<td>$a</td>
						<td>$n</td>
						<td>$r</td>
						<td>$p</td>
						<td>".str_replace("_"," ",$e)."</td>
						<td>
							<input type = 'radio' name ='event$recptno' id =$recptno onclick='Select_events(this.id);' value = 1>Approve</input>
						</td>
					</tr>";
				}			
			}
