<?php
		$hint ="";
		session_start();
		if((isset($_SESSION["name_iso"]))&&(isset($_REQUEST["s"])))
		{
			require 'sql_con.php';
			$s = $_REQUEST["s"];
			...
			if($st==1)
			{
				...
				foreach($name_array as $name)
				{
					...
					{
							echo "<button class='but'><p class='eventbut' style='font-weight:bold;' id=".$id_array[$k]." onclick='getdetails_iso($id_array[$k]);' >".str_replace("_"," ",$name)."<br/>filled seats:".$filled[$k]."<br/>total seats:".$total[$k]."<br/></p></button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";
							$j++;
							if($j%3==0)
								echo "<br/><br/>";
					}
	...
	else if(((!isset($_SESSION["name_iso"]))&&(!isset($_REQUEST["s"])))||((isset($_SESSION["name_iso"]))&&(!isset($_REQUEST["s"]))))
	{
		session_unset();
