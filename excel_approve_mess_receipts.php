<?php
	...
	if((isset($_SESSION["name"])))
	{
		...
		$j=0;
		$t_price=0;
		$query = "SELECT * from temp_registration_mess where query=0 and approval=0";
		...
		$d=date('d/m/Y');
		$file_name = "Approve_Mess_Receipts_$d.xls";
		header( "Content-Type: application/vnd.ms-excel" );
		header( "Content-disposition: attachment; filename=$file_name" );
		echo   'Receipt No' . "\t". 'Name' . "\t". 'Regno' . "\t" . 'PH no' ."\t" . 'Events' . "\t".'Email '."\t". 'Amount' ."\t".'Count' ."\n";
		for($j=0;$j<$i;$j++)
		{
